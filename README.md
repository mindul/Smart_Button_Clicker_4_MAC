...
* 동작 원리 : 
 마우스 포인터가 위치한 곳의 400x400 픽셀 공간을 
...
optional arguments:
  ...
  <최초 실행 시> 
  * 1단계 : '다음 페이지' 버튼 이미지를 마우스로 블럭을 잡아서(드래그&드랍)  jpg 파일로 저장
  $ python3 button_clicker.py -c 
  * 2단계 : 마우스 커서를 "다음 페이지" 버튼 위에 올려두고 사이버교육을 시청한다.
  * 3단계 : 프로그램이 마우스 커서 근처에서 "다음 페이지" 버튼을 찾아 클릭하여 다음 페이지로 넘긴다. 
  끝.